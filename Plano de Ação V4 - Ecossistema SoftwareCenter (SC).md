...
O ecossistema é composto por micro-serviços especializados, divididos entre o núcleo de controle (Core), o gateway de segurança e os motores de negócio que agregam valor à plataforma.
...
graph TD
  ...
  classDef core fill:#0d47a1,stroke:#000,stroke-width:1px,color:#fff;
  classDef front fill:#00695c,stroke:#000,stroke-width:1px,color:#fff;
  classDef engine fill:#e65100,stroke:#000,stroke-width:1px,color:#fff;

  User((Administrador<br/>Tenant)) -->|Acessa| Portal

  subgraph "Borda e Interface"
    Portal[SC Portal<br/>Painel Vue 3]
    AG[SC AG<br/>Auth Gateway]
  end

  Portal -->|HTTPS + Cookies CSRF| AG

  subgraph "Componentes Core"
    AG -->|mTLS + Auth S2S| SCSSO[SC SSO<br/>Identity Provider]
    AG -->|mTLS + JWT Proxy| SC[SC<br/>Control Plane & RBAC]
    SC --- Billing[SC Billing<br/>Motor de Faturamento]
  end

  subgraph "Motores Nativos de Negócio"
    AG -->|JWT Proxy| Sign[SC Sign<br/>Assinatura Qualificada]
    AG -->|JWT Proxy| Wpp[SC Wpp<br/>WhatsApp Cloud API]
  end

  %% Aplicação de classes segura (evita Parse Error)
  class Portal,AG front;
  class SCSSO,SC,Billing core;
  class Sign,Wpp engine;
```
...
### 3.3 Motores Nativos de Negócio

- SC Sign: Um motor especializado em Assinatura Eletrônica Qualificada. Ele confere validade técnica e forense a documentos por meio da geração de hashes SHA-256 ancorados temporalmente via Time-Stamping Authority (RFC 3161), mantendo uma trilha de auditoria imutável que registra IP, consentimento e dados da identidade.

- SC Wpp: Motor responsável pela orquestração e integração dedicada com a API oficial da Meta (WhatsApp Cloud API) para disparos transacionais confiáveis. Baseado em templates aprovados, opera com um sistema de filas estruturado que realiza tentativas de reenvio e executa fallbacks automáticos (para e-mail ou SMS) caso a entrega primária falhe.
...
graph TD
  ...
  SC -.->|Publica Invalidações e CORS PUBLISH| Redis
...
erDiagram
    ...
    TENANTS { bigint id_tenant PK }
    CONTRATOS {
      bigint id_contrato PK
      bigint id_tenant_contratante FK
      string status "ATIVO|SUSPENSO"
      datetime dia_vencimento
    }
    CONTRATO_ITENS {
      bigint id_item PK
    ...
    LICENCAS_M2M {
      bigint id_licenca PK
      bigint id_tenant FK
      string client_id "Referência Lógica -> OAUTH2_CLIENTS"
      string status
    }
      
    %% Relacionamentos a serem adicionados no final do diagrama:
    TENANTS ||--o{ LICENCAS_M2M : "emite"
    LICENCAS_M2M ||--o{ ATRIBUICOES : "recebe cargos (Service Account)"        

    %% Banco de Dados: SC (Metering & Billing)
    TABELA_PRECOS {
      bigint id_preco PK
      string recurso "SC_NOTIFY|SC_SIGN"
      decimal valor_unitario
      datetime data_vigencia_inicio
    }
    REGISTROS_CONSUMO {
      ...
      string recurso
      int quantidade
      decimal valor_unitario_congelado
    ...
    FATURAS {
      ...
      bigint id_contrato FK
      decimal valor_total
      string status "ABERTA|PAGA|VENCIDA"
      datetime competencia
    }

  TENANTS ||--o{ CONTRATOS : "possui MSA"
  CONTRATOS ||--o{ CONTRATO_ITENS : "possui aditivos"
  CONTRATO_ITENS }o--|| APLICACOES : "referencia"
  TENANTS ||--o{ REGISTROS_CONSUMO : "gera consumo"
  CONTRATOS ||--o{ FATURAS : "fatura mensal"
```
...
### 6.5 Integração M2M e Licença Unificada (Service Accounts)

A plataforma suporta a integração direta de sistemas terceiros aos Motores Nativos (ex: SC Sign, SC Notify) através do padrão OAuth2 `client_credentials`, orquestrado pelo motor de RBAC.

- **A Licença Imutável:** O Tenant gera um par de credenciais (`Client ID` e `Client Secret`) que atua como a licença global de integração.
- **Contas de Serviço (RBAC):** A Licença é tratada pelo motor de autorização como um *Usuário Virtual (Service Account)*. O administrador do Tenant utiliza o painel de RBAC para atribuir **Cargos** específicos a essa Licença, restringindo seu escopo.
- **O Token Dinâmico (Mutável):** Ao autenticar a Licença na SC SSO, um token JWT de curta duração (1 hora) é emitido. Este JWT é enriquecido apenas com as permissões dos *Contratos Ativos* e os *Cargos* vinculados àquela Licença no momento exato da requisição.
- **Dogfooding (System Client):** A própria SC utiliza esse mecanismo (Tenant Zero) para consumir os motores nativos em processamentos internos, contendo uma claim `role: ["SYSTEM_INTERNAL"]` que isenta a chamada da esteira de faturamento.

---

### 6.6 Gestão de Chaves M2M (Rolling Keys e Revogação Imediata)

Para mitigar danos em caso de vazamento de credenciais (`Client Secret`) sem causar indisponibilidade (downtime) na integração do Tenant, a SC implementa o padrão **Rolling Keys** e um mecanismo de revogação na borda.

- **Chaves Sobrepostas (Rolling Keys):** O banco de dados (`LICENCAS_M2M`) permite múltiplas chaves ativas simultaneamente para o mesmo contexto. O Tenant pode gerar uma "Chave B", configurar em sua infraestrutura, validar o funcionamento e só então revogar a "Chave A" que estava comprometida ou em ciclo de rotação.
- **Revogação Imediata (Botão de Pânico):** Ao revogar uma chave no painel da SC, o sistema executa duas ações simultâneas:
    1. *Soft-delete* da chave no banco de dados da SC e inativação no Authorization Server (SC SSO).
    2. *Publicação de Evento (Pub/Sub):* A SC dispara um evento imediato de revogação no **Redis** (ex: `revoke:client_id:XYZ`).
- **Bloqueio na Borda (SC AG):** O SC Auth Gateway lê eventos de revogação em tempo real. Qualquer requisição que apresente um JWT cujo `subject` (Client ID) conste na lista de revogação recente é rejeitada com `401 Unauthorized` instantaneamente, ignorando o tempo de expiração natural (TTL) do token.
...
| UC-007 | Gestão de RBAC (Cargos, Projetos e Permissões) | Admin | Capability `RBAC_MANAGE` | Token delegado `sc-management` → CRUD com base nas relações: Cargo atua como Alias; Projeto contém N RBACs; 1 RBAC aponta para 1 Cargo; Usuário (ou Licença M2M) possui N Cargos (permitindo papéis divergentes em módulos distintos). | Não delega permissão superior à efetiva; inativações ocorrem via versionamento de manifesto. | REQ-RBAC |
| UC-016 | Rotação de Chave M2M (Rolling Keys) | Admin | Chave anterior ativa | Gera nova chave com os mesmos vínculos (RBAC) da chave anterior; mantém ambas ativas para transição sem downtime. | Limite máximo de chaves simultâneas excedido. | REQ-M2M |
| UC-017 | Revogação Imediata de Licença | Admin / Automação | Chave existente | Inativa chave no banco; publica evento no Redis; SC AG passa a rejeitar tokens ativos derivados da chave. | Chave já revogada. | REQ-M2M |
...
### 13.5 Motor de Faturamento Consolidado (Metering & Billing Engine)

Para garantir uma experiência de usuário (UX) premium sem atritos, a plataforma opera no modelo **Pós-Pago Consolidado**. Todas as licenças fixas e consumos variáveis (SC Notify, SC Sign) são agrupados em uma única Licença (Fatura Mensal) por Tenant.

| Característica | Regra de Negócio |
| :--- | :--- |
| **Contrato Master (MSA)** | O Tenant assina um contrato guarda-chuva. Novas contratações de aplicações geram "Aditivos" (Itens de Contrato), com cobrança *Pró-rata* (proporcional aos dias de uso) alinhada à data de vencimento principal. |
| **Congelamento de Consumo** | Cada uso de recurso variável gera um registro no banco de dados gravando a quantidade e o **preço unitário exato do momento da requisição**. Isso garante a legalidade da fatura e evita recálculos retroativos. |
| **Fechamento e Fatura Única** | Todo dia 1º (ou na data de corte), um *Cron Job* na SC soma as mensalidades ativas + o consumo do mês anterior, emite a NF-e detalhada e gera a cobrança via Gateway parceiro. |
| **Reajuste de Preços** | Tabelas de preços possuem versionamento por `data_vigencia_inicio`. Reajustes disparam notificações automáticas aos Tenants com 30 dias de antecedência (conformidade legal B2B). |
| **Inadimplência (Lockout)** | Em caso de não pagamento após a janela de tolerância, a SC publica o status de bloqueio no Redis. O SC AG barra imediatamente o acesso do Tenant a todas as aplicações. |
...
| Inadimplência no modelo Pós-Pago | Alta | Médio | Bloqueio automatizado na borda (SC AG) após *grace period*; cláusula penal no contrato MSA aceito. | Financeiro |
| Questionamento jurídico de reajustes | Baixa | Médio | Versionamento da tabela de preços com `data_vigencia` + log inalterável de notificação com 30 dias de antecedência. | Legal / Product |
| Vazamento de Licença M2M em repositório de código | Alta | Alto | Suporte a **Rolling Keys** (troca sem downtime) + Revogação instantânea na borda com invalidação de JWT via Redis (§6.6). | Security / Tenant |
